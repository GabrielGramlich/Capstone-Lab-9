...
class TestWishList(TestCase):
	...
	def test_view_wishlist_contains_not_visited_places(self):
		...
		self.assertNotContains(response, 'San Fransisco')
		self.assertNotContains(response, 'Moab')


class TestVisitedPage(TestCase):
	def test_load_visited_page_shows_empty_list_for_empty_database(self):
...
class TestPlacesVisited(TestCase):
	...
	def test_view_places_visited_contains_visited_places(self):
		response = self.client.get(reverse('places_visited'))
		self.assertTemplateUsed(response, 'travel_wishlist/visited.html')

		self.assertNotContains(response, 'Tokyo')
		self.assertNotContains(response, 'New York')
		self.assertContains(response, 'San Fransisco')
